...
boost::asio::awaitable<void> listener(boost::asio::ip::tcp::acceptor acceptor)
{
    ...
    while (true)
    {
        auto session = std::make_shared<World::Session>(co_await acceptor.async_accept(boost::asio::use_awaitable));
        session->start();
    }
}
...
int main()
{
    try
    {
        ...
        auth_database->execute(
            fmt::format("UPDATE realmlist SET flags = flags | {} WHERE id = {}", Realm::realmflag_offline, 1).c_str());

        boost::asio::io_context io_context(1);

        boost::asio::co_spawn(io_context,
                              listener(boost::asio::ip::tcp::acceptor(io_context, {boost::asio::ip::tcp::v4(), 8085})),
                              boost::asio::detached);
